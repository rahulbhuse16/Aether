...
export interface ChatMessage {
  ...
  text: string;
  timestamp: string;
  /** Where the message originated — lets the UI badge Slack-side replies. */
  source: "app" | "slack";
}
...
export const slackService = {
  ...
  /**
   * Chat: two-way message sync with a Slack channel.
   */
  getChannelMessages: async (
    ...
    const { data } = await slackApi.get(`/slack/channels/${channelId}/messages`, {
  ...
  sendChannelMessage: async (
    ...
    const { data } = await slackApi.post(`/slack/channels/${channelId}/messages`, {
      userId,
      text,
    });
    return data;
