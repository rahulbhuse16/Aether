...
exports.sendDailySummaryNow = exports.getDailySummary = exports.getGithubNotifications = exports.getBugAnalyses = exports.getTasks = exports.getMentions = void 0;
...
const { SLACK_SIGNING_SECRET } = env_1.ENV;
/**
 ...
 */
async function getProjectContextForUser(userId) {
    try {
    ...
    try {
        ...
        const computedSignature = "v0=" +
            crypto_1.default
                ...
                .update(baseString)
                .digest("hex");
        const isValid = crypto_1.default.timingSafeEqual(Buffer.from(computedSignature), Buffer.from(signature));
        if (!isValid) {
 ...
 */
async function processMention(event, teamId) {
    ...
    try {
        ...
        if (!messageText) {
            await slack_1.slackMessagingService.postText(accessToken, event.channel, "What would you like help with? Try: \"analyze issue #142 in <repo>\", \"create a task: ...\", or paste an error.", threadTs);
            return;
        }
        // Fetch user's project context to ground all AI responses
        const projects = await getProjectContextForUser(userIdStr);
        const classification = await groq_1.groqService.classifyMention(messageText, projects);
        switch (classification.intent) {
            case "analyze_issue": {
                if (!classification.issueNumber) {
                    await slack_1.slackMessagingService.postText(accessToken, event.channel, "Which issue number should I look at?", threadTs);
                    break;
                }
                // Auto-detect the repo from what the user typed (or fall
                // back to their only connected repo, if they have just one).
                const repoFullName = await github_service_1.githubService.resolveRepo(userIdStr, classification.repoHint);
                if (!repoFullName) {
                    const repoNames = await github_service_1.githubService.listRepoNames(userIdStr);
                    const suggestion = repoNames.length > 0
                    ...
                    await slack_1.slackMessagingService.postText(accessToken, event.channel, `I need to know which repo — ${suggestion}`, threadTs);
                    break;
                }
                const issueContext = await github_service_1.githubService.getIssueContext(userIdStr, classification.issueNumber, repoFullName);
                if (!issueContext) {
                    await slack_1.slackMessagingService.postText(accessToken, event.channel, `I couldn't find issue #${classification.issueNumber} in ${repoFullName}.`, threadTs);
                    break;
                }
                // Pull the actual source files most relevant to this issue
                // so the fix is grounded in real code, not a guess.
                const repoCode = await github_service_1.githubService.getRepoCodeContext(userIdStr, `issue #${classification.issueNumber}: ${issueContext.slice(0, 300)}`, repoFullName);
                ...
                const notionCtx = await (0, notion_1.getRelevantNotionContext)(userIdStr, `${repoFullName} ${issueContext.slice(0, 200)}`);
                const analysis = await groq_1.groqService.analyzeGithubIssue(issueContext, projects, repoCode, notionCtx.contextBlock);
                await slack_1.slackMessagingService.postIssueAnalysis(accessToken, event.channel, classification.issueNumber, analysis, threadTs);
                break;
            }
            case "create_task": {
                const title = classification.taskTitle?.trim();
                if (!title) {
                    await slack_1.slackMessagingService.postText(accessToken, event.channel, "I need a task title — try \"@Aether create a task: <title>, priority high\".", threadTs);
                    break;
                }
                /**
                 ...
                 */
                const defaultProject = await project_1.Project.findOne({ owner: user._id }).sort({
                    createdAt: 1,
                });
                if (!defaultProject) {
                    await slack_1.slackMessagingService.postText(accessToken, event.channel, "You don't have any projects in Aether yet — create one first, then I can add tasks to it from Slack.", threadTs);
                    break;
                }
                const task = await task_1.Task.create({
                    id: `slack-${crypto_1.default.randomUUID()}`,
                    title,
                    status: "open",
                    source: "slack",
                ...
                await slack_1.slackMessagingService.postTaskCreated(accessToken, event.channel, {
                    title: task.title,
            ...
            case "bug_report": {
                ...
                const repoCode = repoFullName
                    ? await github_service_1.githubService.getRepoCodeContext(userIdStr, messageText, repoFullName)
                    : null;
                const notionCtx = await (0, notion_1.getRelevantNotionContext)(userIdStr, messageText);
                const analysis = await groq_1.groqService.analyzeBugReport(messageText, projects, repoCode, notionCtx.contextBlock);
                await slack_1.slackMessagingService.postBugAnalysis(accessToken, event.channel, analysis, threadTs);
                break;
            }
            ...
            default: {
                ...
                const repoFullName = classification.repoHint
                    ? await github_service_1.githubService.resolveRepo(userIdStr, classification.repoHint)
                    : null;
                const repoCode = repoFullName
                    ? await github_service_1.githubService.getRepoCodeContext(userIdStr, messageText, repoFullName)
                    : null;
                const notionCtx = await (0, notion_1.getRelevantNotionContext)(userIdStr, messageText);
                const answer = await groq_1.groqService.answerGeneralQuestion(messageText, projects, repoCode, notionCtx.contextBlock);
                await slack_1.slackMessagingService.postText(accessToken, event.channel, answer, threadTs);
                break;
            }
        }
    }
        ...
        await slack_1.slackMessagingService.postText(accessToken, event.channel, "Something went wrong on my end processing that — mind trying again in a moment?", threadTs);
    }
}
/**
 * -----------------------------------------------------------------------
...
async function fetchChannelTranscript(client, channelId, channelName, oldestTs) {
...
async function getFullTranscript(client, sinceDaysAgo) {
    const oldestTs = ((Date.now() - sinceDaysAgo * 24 * 60 * 60 * 1000) /
        1000).toString();
    const channels = await listBotChannels(client);
    const transcripts = await Promise.all(channels.map((c) => fetchChannelTranscript(client, c.id, c.name, oldestTs)));
    return transcripts.filter(Boolean).join("\n");
}
function getSlackClientForUser(user) {
    if (!user?.slack?.connected || !user.slack.accessToken)
        return null;
    return new web_api_1.WebClient(user.slack.accessToken);
}
...
{ "mentions": [ { "channel": string, "userName": string, "question": string, "response": string, "confidence": number | null, "relatedGithubIssue": string | null, "timestamp": string } ] }`;
        const { mentions } = await extractJson(system, transcript);
        res.status(200).json(mentions.map((m, i) => ({ id: `mn-${i}-${m.timestamp}`, ...m })));
    }
    catch (error) {
...
{ "tasks": [ { "title": string, "priority": "low" | "medium" | "high", "createdBy": string, "createdAt": string } ] }`;
        const { tasks } = await extractJson(system, transcript);
        res.status(200).json(tasks.map((t, i) => ({
            id: `tk-${i}-${t.createdAt}`,
            status: "open",
            source: "slack",
            ...t,
...
const getBugAnalyses = async (req, res) => {
    try {
        const { userId } = req.query;
        if (!userId) {
            res.status(400).json({ message: "userId is required" });
...
{ "bugAnalyses": [ { "channel": string, "userName": string, "errorSnippet": string, "rootCause": string, "recommendedFix": string, "confidence": number, "timestamp": string } ] }`;
        const { bugAnalyses } = await extractJson(system, transcript);
        res.status(200).json(bugAnalyses.map((b, i) => ({ id: `bg-${i}-${b.timestamp}`, ...b })));
    }
    catch (error) {
...
exports.getBugAnalyses = getBugAnalyses;
const getGithubNotifications = async (req, res) => {
    try {
        const { userId } = req.query;
        if (!userId) {
            res.status(400).json({ message: "userId is required" });
...
{ "notifications": [ { "repository": string, "issueTitle": string, "priority": "low" | "medium" | "high", "assignedTo": string, "aiAnalysis": string, "timestamp": string } ] }`;
        const { notifications } = await extractJson(system, transcript);
        res.status(200).json(notifications.map((n, i) => ({
            id: `gn-${i}-${n.timestamp}`,
            githubUrl: "",
            aetherUrl: "",
            ...n,
...
exports.getGithubNotifications = getGithubNotifications;
async function buildDailySummary(client) {
    const startOfDayTs = (new Date(new Date().setHours(0, 0, 0, 0)).getTime() / 1000).toString();
    const channels = await listBotChannels(client);
    const todaysMessages = (await Promise.all(channels.map((c) => client.conversations.history({
        channel: c.id,
        oldest: startOfDayTs,
        limit: MAX_MESSAGES_PER_CHANNEL,
    ...
    const countMatching = (list, pattern) => list.filter((m) => pattern.test(m.text || "")).length;
    const stats = {
    ...
    const topIssueTitles = botMessages
        ...
        .slice(0, 10)
        .map((m) => (m.text || "").split("\n")[0]);
    const insights = Object.values(stats).every((v) => v === 0)
        ? { insights: [], recommendedAction: "No notable activity in Slack today." }
        : await groq_1.groqService.generateDailySummaryInsights({
            ...stats,
            topIssueTitles,
        });
...
const sendDailySummaryNow = async (req, res) => {
    try {
        ...
        const { stats, insights } = await buildDailySummary(client);
        await slack_1.slackMessagingService.postDailySummary(user.slack.accessToken, channelId, stats, insights);
