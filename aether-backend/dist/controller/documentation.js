...
exports.regenerateDoc = regenerateDoc;
exports.getLatestDocsSession = getLatestDocsSession;
...
const DOC_ORDER = ["readme", "api", "architecture", "flow"];
const DOC_META = {
...
async function fetchFileContent(owner, repo, path, branch, token) {
    try {
        ...
        console.warn(`[docs] failed to fetch ${path}:`, err.message);
        return null;
    }
}
...
async function buildDocsContext(owner, repo, branch, token) {
...
function isValidLLMOutput(value, expectedTypes) {
    if (!value || !Array.isArray(value.documents))
        return false;
    const returnedTypes = value.documents.map((d) => d?.type);
    return (expectedTypes.every((t) => returnedTypes.includes(t)) &&
        value.documents.every((d) => d && typeof d.type === "string" && typeof d.content === "string" && d.content.trim().length > 0));
}
...
// Groq call
// -----------------------------------------------------------------------------
async function callGroqForDocs(contextText, owner, repo, branch, requestedTypes) {
    ...
    for (let attempt = 1; attempt <= GROQ_MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            lastError = err;
            console.warn(`[docs] Groq call attempt ${attempt} failed:`, err.message);
        }
    }
...
function normalizeDocType(type) {
    ...
    return DOC_ORDER.includes(lower) ? lower : null;
}
function buildDoc(type, content, index) {
    return {
...
async function generateDocs(req, res) {
    try {
        ...
        try {
            ...
            const info = err?.groqError;
            if (info && (info.kind === "rate_limit_day" || info.kind === "rate_limit_minute")) {
                return rateLimitResponse(res, info);
            }
            await documentation_1.DocsSession.findOneAndUpdate({ user: userId, repoId: String(repoId) }, {
                user: userId,
        ...
        const documents = llmDocs
            ...
            .filter((d) => d !== null);
        const session = await documentation_1.DocsSession.findOneAndUpdate({ user: userId, repoId: String(repoId) }, {
            user: userId,
            repoId: String(repoId),
            owner,
            repoName: repo,
 ...
 */
async function regenerateDoc(req, res) {
    try {
        ...
        const patched = await documentation_1.DocsSession.findOneAndUpdate({ user: userId, repoId: String(repoId), "documents.type": type }, { $set: { "documents.$": newDoc } }, { new: true });
 ...
 */
async function getLatestDocsSession(req, res) {
    try {
        ...
        const session = await documentation_1.DocsSession.findOne({ user: userId, repoId: String(repoId) });
