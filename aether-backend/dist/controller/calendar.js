...
exports.createGoogleMeeting = exports.getCalendarEvents = exports.getCalendarStatus = exports.googlewebhook = exports.googleCalendarCallback = exports.connectGoogle = void 0;
...
const { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, CALENDAR_REDIRECT_URI, GOOGLE_WEBHOOK_URL, FRONTEND_URL, } = env_1.ENV;
...
const oauth2Client = new googleapis_1.google.auth.OAuth2(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, CALENDAR_REDIRECT_URI);
const GOOGLE_SCOPES = [
 ...
 */
const getAuthClientForUser = (user) => {
    const auth = new googleapis_1.google.auth.OAuth2(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, CALENDAR_REDIRECT_URI);
    auth.setCredentials({
...
const googlewebhook = async (req, res) => {
    try {
         ...
         */
        if (user.googleCalendar.channelId &&
            channelId &&
 ...
 */
const createGoogleCalendarWatch = async (userId) => {
    try {
        const user = await user_1.User.findById(userId);
        if (!user?.googleCalendar?.accessToken) {
...
const getCalendarStatus = async (req, res) => {
    try {
        ...
        const user = await user_1.User.findById(userId).select("email googleCalendar");
        if (!user) {
            res.status(404).json({
...
const getCalendarEvents = async (req, res) => {
    try {
        ...
        const now = Date.now();
        const startTime = typeof timeMin === "string"
            ? timeMin
            : new Date(now).toISOString();
        const endTime = typeof timeMax === "string"
            ? timeMax
...
const createGoogleMeeting = async (req, res) => {
    try {
        const { userId, title, description, startTime, endTime, timezone = "Asia/Kolkata", attendees = [], } = req.body;
        if (!userId ||
            !title ||
            !startTime ||
            !endTime) {
