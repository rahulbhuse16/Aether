...
async function fetchUserRepos(token) {
    const repos = await githubFetch(token, "/user/repos?sort=updated&per_page=50&affiliation=owner,collaborator");
    return repos.map((r) => ({
        id: r.id,
        name: r.full_name,
 ...
 */
async function gatherRepoSignals(token, repoId) {
    ...
    const languages = languagesRes.status === "fulfilled" ? Object.keys(languagesRes.value) : [];
    const topLevelFiles = treeRes.status === "fulfilled"
        ? treeRes.value.tree
    ...
    if (readmeRes.status === "fulfilled") {
        const raw = readmeRes.value.content;
        if (raw) {
...
function isValidBrief(x) {
    return (typeof x?.summary === "string" &&
        x.summary.trim().length > 0 &&
        Array.isArray(x?.stack) &&
        x.stack.every((tag) => typeof tag === "string") &&
        ["low", "medium", "high"].includes(x?.setupComplexity));
}
async function callOnboardingModel(signals) {
    let lastError;
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            if (!isValidBrief(parsed))
                throw new Error("Onboarding brief missing required fields");
            ...
            const haystack = [
                ...
                .join(" ")
                .toLowerCase();
            parsed.stack = parsed.stack.filter((tag) => haystack.includes(tag.toLowerCase()));
            return parsed;
        }
        catch (err) {
            lastError = err;
...
async function indexRepository(req, res) {
    ...
    try {
        ...
        const existing = await project_1.Project.findOne({
            owner: userId,
        ...
        const project = existing
            ? await project_1.Project.findByIdAndUpdate(existing._id, projectFields, { new: true })
            : await project_1.Project.create(projectFields);
        res.json({
