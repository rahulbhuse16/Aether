...
// Config
// -----------------------------------------------------------------------------
const VALID_NODE_TYPES = ["frontend", "gateway", "service", "database", "cache", "queue"];
const MAX_NODES = 14;
const MIN_NODES = 5;
...
function isValidLLMOutput(value) {
    return (!!value &&
        ...
        value.nodes.every((n) => n && typeof n.label === "string" && typeof n.type === "string"));
}
function normalizeNodeType(type) {
    const lower = (type || "").toLowerCase().trim();
    return VALID_NODE_TYPES.includes(lower) ? lower : "service";
}
...
function mapSuggestions(suggestions, prompt) {
    ...
    for (const raw of suggestions) {
        ...
        if (!s || key === promptLower || seen.has(key))
            continue;
        seen.add(key);
        cleaned.push(s);
        if (cleaned.length >= MAX_SUGGESTIONS)
            break;
    }
...
// Groq call
// -----------------------------------------------------------------------------
async function generateArchitectureFromPrompt(prompt) {
    ...
    for (let attempt = 1; attempt <= GROQ_MAX_RETRIES; attempt++) {
        try {
            ...
            lastError = err;
            console.warn(`[architecture] Groq call attempt ${attempt} failed:`, err.message);
        }
    }
