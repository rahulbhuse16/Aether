...
exports.generateVoiceCommand = generateVoiceCommand;
exports.getVoiceHistory = getVoiceHistory;
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const env_1 = require("../config/env");
const voice_engineer_1 = require("../models/voice-engineer");
...
function isValidGeneratedFile(value) {
    return (!!value &&
        typeof value === "object" &&
...
// Groq call
// -----------------------------------------------------------------------------
async function generateVoiceEngineerResponse(transcript) {
    ...
    for (let attempt = 1; attempt <= GROQ_MAX_RETRIES; attempt++) {
        try {
            ...
            lastError = err;
            console.warn(`[voice-engineer] Groq call attempt ${attempt} failed:`, err.message);
        }
    }
 ...
 */
async function generateVoiceCommand(req, res) {
    try {
        ...
        const command = await voice_engineer_1.VoiceCommand.create({
            userId: userId || null,
        ...
        try {
            ...
            await voice_engineer_1.VoiceCommand.findByIdAndUpdate(command._id, {
                status: "complete",
        ...
        const updatedCommand = await voice_engineer_1.VoiceCommand.findByIdAndUpdate(command._id, {
            status: "complete",
...
async function getVoiceHistory(req, res) {
    try {
        ...
        const commands = await voice_engineer_1.VoiceCommand.find(filter)
            .sort({ createdAt: -1 })
            .limit(20);
        return res.status(200).json({
            success: true,
