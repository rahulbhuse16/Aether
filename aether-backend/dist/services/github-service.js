 ...
 */
async function resolveRepo(userId, hint) {
    const repos = await exports.githubService.listAccessibleRepos(userId);
    if (repos.length === 0)
        return null;
    if (!hint || !hint.trim()) {
    ...
    if (exactFullName)
        return exactFullName.fullName;
    const exactRepoName = repos.find((r) => r.fullName.toLowerCase().split("/")[1] === cleanHint);
    if (exactRepoName)
        return exactRepoName.fullName;
 ...
 */
async function getRepoCodeContextInternal(githubToken, repoFullName, query) {
    try {
