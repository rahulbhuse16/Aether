...
function buildProjectContextBlock(projects) {
    ...
    const lines = projects.map((p) => {
        ...
        if (p.stack.length > 0)
            parts.push(`Tech stack: ${p.stack.join(", ")}`);
 ...
 */
function buildRepoCodeContextBlock(repoCode) {
    if (!repoCode || repoCode.files.length === 0)
        return "";
 ...
 * Notion at all.
 */
function buildNotionContextBlock(notionContext) {
...
exports.groqService = {
     ...
     */
    classifyMention: async (messageText, projects) => {
...
Respond ONLY with a JSON object matching exactly:
{
  ...
  "repoHint": string | null
        ...
        return completeJson(system, messageText);
    },
    /**
     ...
     */
    analyzeGithubIssue: async (issueContext, projects, repoCode, notionContext) => {
        const projectCtx = buildProjectContextBlock(projects);
...
2. Identify the most specific, likely root cause. ${repoCode
...
Respond ONLY with a JSON object, no prose, no markdown fences, matching exactly:
{
  ...
  "confidence": number
        ...
        return completeJson(system, issueContext);
    },
    /**
     ...
     * generic hypothesis.
     */
    analyzeBugReport: async (errorAndCode, projects, repoCode, notionContext) => {
        const projectCtx = buildProjectContextBlock(projects);
        const repoCtx = buildRepoCodeContextBlock(repoCode);
        const notionCtx = buildNotionContextBlock(notionContext);
...
Respond ONLY with a JSON object, no prose, no markdown fences, matching exactly:
{
  ...
  "confidence": number
        ...
        return completeJson(system, errorAndCode);
    },
    /**
     ...
     * hallucinated.
     */
    generateDailySummaryInsights: async (input, projects) => {
        const projectCtx = buildProjectContextBlock(projects);
...
Respond ONLY with a JSON object, no prose, no markdown fences, matching exactly:
{
  ...
  "recommendedAction": string
        ...
        return completeJson(system, JSON.stringify(input));
    },
    /**
     ...
     */
    answerGeneralQuestion: async (messageText, projects, repoCode, notionContext) => {
        const projectCtx = buildProjectContextBlock(projects);
        const repoCtx = buildRepoCodeContextBlock(repoCode);
        const notionCtx = buildNotionContextBlock(notionContext);
...
${projectCtx}${repoCtx}${notionCtx}`;
     ...
     */
    extractActionItemsFromNotes: async (notesContent) => {
...
Respond ONLY with a JSON object, no prose, no markdown fences, matching exactly:
{
  ...
  "severity": "low" | "medium" | "high" | "critical"
        ...
        return completeJson(system, JSON.stringify(payload));
    },
};
