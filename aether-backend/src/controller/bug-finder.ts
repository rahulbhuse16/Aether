...
import { BugAnalysis, IBugFinding, Severity } from "../models/bug-analysis";
...
function isRelevantFile(path: string): boolean {
  ...
  if (IGNORED_PATH_SEGMENTS.some((seg) => lower.includes(seg))) return false;
  return RELEVANT_EXTENSIONS.some((ext) => lower.endsWith(ext));
}

function pickFilesToAnalyze(
  ...
  if (focusPath) {
    ...
    if (scoped.length > 0) blobs = scoped;
  }
...
async function fetchFileContent(
  ...
  try {
    ...
    console.warn(`[bugfinder] failed to fetch ${path}:`, (err as Error).message);
    return null;
  }
}
...
async function buildRepositoryContext(
  ...
  token: string,
  focusPath?: string
): Promise<{ files: RepoFile[]; filesAnalyzed: number; filesSkipped: number }> {
...
function formatContextForModel(files: RepoFile[]): string {
  return files
    .map((f) => `// FILE: ${f.path}\n${f.content}\n// END FILE: ${f.path}`)
    .join("\n\n");
}
...
function isValidAiResponse(value: any): value is AiResponse {
  return (
    !!value &&
...
function isRequestTooLargeError(err: unknown): boolean {
  ...
  const code = (err as any)?.error?.error?.code ?? (err as any)?.code;
 ...
 */
async function callGroq(userPrompt: string): Promise<AiResponse> {
  if (!ENV.GROQ_API_KEY) {
  ...
  for (let attempt = 1; attempt <= GROQ_MAX_RETRIES; attempt++) {
    try {
      ...
      if (isRequestTooLargeError(err)) {
        console.warn(
          `[bugfinder] Groq call attempt ${attempt} failed: request too large for the current TPM limit. Shrinking context and retrying.`
        );
        currentPrompt = shrinkPrompt(currentPrompt, GROQ_SHRINK_FACTOR);
        lastError = new Error(
          "Repository context is too large for the current Groq plan's tokens-per-minute limit, even after truncation. " +
...
function normalizeSeverity(value: string): Severity {
  ...
  return (VALID_SEVERITIES as string[]).includes(lower) ? (lower as Severity) : "info";
}

function mapAiFindingToFinding(f: AiFinding): IBugFinding {
  return {
...
function serializeReport(doc: InstanceType<typeof BugAnalysis>) {
  return {
    id: doc._id.toString(),
    repoUrl: doc.repoUrl,
    ...
    findings: doc.findings.map((f) => ({
      id: (f as any)._id?.toString?.() ?? undefined,
      title: f.title,
      severity: f.severity,
...
export async function analyzeRepository(req: Request, res: Response) {
  try {
    ...
    const { repoId, branch, focusPath, stackTrace }: AnalyzeRequestBody = req.body;
    ...
    let owner: string, repo: string, defaultBranch: string, htmlUrl: string;
    try {
    ...
    const { files, filesAnalyzed, filesSkipped } = await buildRepositoryContext(
      owner,
      repo,
      resolvedBranch,
      token,
    ...
    try {
      ...
      const failureMessage = (err as Error).message || "AI analysis failed";
      const failed = await BugAnalysis.create({
        user: userId,
        repoUrl: htmlUrl,
        repoName: repo,
        owner,
    ...
    const saved = await BugAnalysis.create({
      user: userId,
      repoUrl: htmlUrl,
      repoName: repo,
      owner,
      ...
      repositoryHealthScore:
        typeof aiResult.repositoryHealthScore === "number"
    ...
    return res.status(500).json({
      success: false,
      message: (err as Error).message || "Repository analysis failed",
    });
  }
}
...
export async function getReports(req: Request, res: Response) {
  try {
    ...
    const reports = await BugAnalysis.find({ user: userId })
      .sort({ createdAt: -1 })
      .limit(50);
...
export async function getReportById(req: Request, res: Response) {
  try {
    ...
    const report = await BugAnalysis.findOne({ _id: req.params.id, user: userId });
    if (!report) {
...
export async function deleteReport(req: Request, res: Response) {
  try {
    ...
    const deleted = await BugAnalysis.findOneAndDelete({
      _id: req.params.id,
      user: userId,
    });
