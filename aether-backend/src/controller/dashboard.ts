...
interface DigestPrediction {
  title: string;
  description: string; // multi-line, bullet-pointed, 6-8 lines
  estimatedMinutes: number;
  severity: "low" | "medium" | "high";
}

interface DigestModelOutput {
  yesterday: string; // multi-line, bullet-pointed, 6-8 lines
...
function isValidOutput(x: any): x is DigestModelOutput {
  if (typeof x?.yesterday !== "string" || x.yesterday.trim().length === 0) return false;
  if (typeof x?.prediction?.title !== "string") return false;
  if (typeof x?.prediction?.description !== "string" || x.prediction.description.trim().length === 0) return false;
  if (!["low", "medium", "high"].includes(x?.prediction?.severity)) return false;

  // Both fields should read as a bulleted brief, not a single sentence.
  if (bulletLineCount(x.yesterday) < 1) return false;
  if (bulletLineCount(x.prediction.description) < 1) return false;
...
async function callDigestModel(signals: GithubSignals): Promise<DigestModelOutput> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (!isValidOutput(parsed)) throw new Error("Digest response missing required fields or not bulleted");
