...
function scoreFilePath(path: string, terms: string[]): number {
  ...
  for (const term of terms) {
    if (base.includes(term)) score += 3; // filename match is a strong signal
    else if (lower.includes(term)) score += 1; // directory/path match, weaker
  }
  return score;
}
...
async function fetchRepoTreePaths(token: string, repoFullName: string, branch: string): Promise<string[]> {
...
/* ---------------------------------------------------------------- */
/* AI — grounded answer + verified citations                         */
/* ---------------------------------------------------------------- */
...
async function callChatModel(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (context.readmeExcerpt) validPaths.add("README.md");
      parsed.citedFiles = parsed.citedFiles.filter((p: string) => validPaths.has(p));
...
export async function sendRepoChatMessage(req: Request, res: Response) {
  const { projectId, message, history,userId } = req.body as {
    projectId?: string;
    message?: string;
    history?: { role: "user" | "assistant"; content: string }[];
    userId?: string;
